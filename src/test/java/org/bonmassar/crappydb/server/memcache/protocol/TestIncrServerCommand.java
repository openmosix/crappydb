...
public class TestIncrServerCommand extends TestCase {

	private IncrServerCommand command;
	private StorageAccessLayer storage;
	private OutputCommandWriter output;
	
	@Before
	public void setUp() {
		command = new IncrServerCommand();
	...
	public void testToString() throws ErrorException {
		...
		assertEquals("{Incr key=terminenzio value=12 noreply=false}", command.toString());
	}
	
	@Test
	public void testToStringNoReply() throws ErrorException {
		command.parseCommandParams("terminenzio 12 noreply\r\n");
		assertEquals("{Incr key=terminenzio value=12 noreply=true}", command.toString());
	}
	...
	public void testError() throws ErrorException, NotFoundException, StorageException {
		...
		doThrow(exception).when(storage).increase(new Key("terminenzio"), "12");
