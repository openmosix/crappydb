...
public class TestPrependServerCommand extends TestCase {
	
	private PrependServerCommand command;
	...
	@Test 
	public void testPrependExceptionOnStorage() throws ErrorException, StorageException, NotFoundException {
		command.parseCommandParams("terminenzio 12 48 50 noreply\r\n");
		command.addPayloadContentPart("012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789".getBytes());
		assertEquals(50, command.payloadCursor);
		
		CrappyDBException exception = new StorageException("BOOM!");
		doThrow(exception).when(storage).prepend((Item) anyObject());
		
		command.execCommand();
		
		verify(storage, times(1)).prepend((Item) anyObject());
		verify(output, times(1)).writeException(exception);
	}
	
	@Test 
	public void testPrependExceptionNotStored() throws ErrorException, StorageException, NotFoundException {
		command.parseCommandParams("terminenzio 12 48 50 noreply\r\n");
		command.addPayloadContentPart("012345678901234567890123456789012345678901234567890123456789012345678901234567890123456789".getBytes());
		assertEquals(50, command.payloadCursor);
	...
	public void testToString() throws ErrorException {
		...
		assertEquals("{Prepend key=terminenzio flags=12 expire=48 nbytes=50 noreply=true} {MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MDEyMzQ1Njc4OTAxMjM0NTY3ODk=}", command.toString());
	}
	...
	public void testToStringNoReply() throws ErrorException {
		...
		assertEquals("{Prepend key=terminenzio flags=12 expire=48 nbytes=50 noreply=false} {MDEyMzQ1Njc4OTAxMjM0NTY3ODkwMTIzNDU2Nzg5MDEyMzQ1Njc4OTAxMjM0NTY3ODk=}", command.toString());
	}
	...
	public void testToStringNoReplyNoPayload() throws ErrorException {
		...
		assertEquals("{Prepend key=terminenzio flags=12 expire=48 nbytes=50 noreply=false}", command.toString());
	}
}
